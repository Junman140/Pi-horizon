...
func createRootCmd(horizonConfig *horizon.Config, configOptions config.ConfigOptions) *cobra.Command {
	return &cobra.Command{
		Use:           "horizon",
		Short:         "client-facing api server for the Stellar/Pi network",
		SilenceErrors: true,
		SilenceUsage:  true,
		Long: "Client-facing API server for the Stellar/Pi network. It acts as the interface between Stellar Core " +
			"and applications that want to access the Stellar/Pi network. It allows you to submit transactions to the " +
